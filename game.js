...
// Load all 12 frames of the slime animation
(async function loadSlimeImages() {
  for (let i = 1; i <= 12; i++) {
    slimeImages.push(await loadImg(`img/slime/slm${i}.png`));
  }
})();
...
function updateEnemies() {
  ...
  const desired = 3 + Math.floor(score / 10);
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    // Update enemy animation frame based on time
    if (now - e.lastFrameTime > e.frameSpeed) {
      e.currentFrame = (e.currentFrame + 1) % 12; // Cycle through 12 frames
      e.lastFrameTime = now;
    }

    // Move the enemy only when the current frame is between 3 and 9 (inclusive)
    if (e.currentFrame >= 3 && e.currentFrame <= 9) {
...
function drawEnemies() {
  for (const e of enemies) {
    const img = slimeImages[e.currentFrame]; // Get the current frame for the enemy

    if (img) {
      ctx.drawImage(img, e.x, e.y, e.size, e.size); // Draw the slime image at the correct position
    }
